          ...
          <View style={graphStyles.graph}>
            {data.map((result, index) => (
              <View key={index} style={graphStyles.barWrapper}>
                ...
                {index % labelStride === 0 && <Text style={graphStyles.barLabel}>{result.orbs}</Text>}
              </View>
            ))}
          </View>
        </View>
...
const graphStyles = StyleSheet.create({ graphContainer: { width: '100%', maxWidth: 800, marginTop: 40, alignItems: 'center' }, title: { fontSize: 24, fontWeight: '600', marginBottom: 4, color: '#333' }, subtitle: { fontSize: 14, fontStyle: 'italic', marginBottom: 16, color: '#666' }, graphLayout: { width: '100%', flexDirection: 'row', marginTop: 10 }, yAxisContainer: { height: 300, justifyContent: 'space-between', paddingRight: 8 }, yAxisLabel: { fontSize: 12, textAlign: 'right', color: '#999' }, graph: { flex: 1, flexDirection: 'row', alignItems: 'flex-end', height: 300, borderLeftWidth: 1, borderBottomWidth: 1, paddingTop: 10, borderColor: '#ccc' }, barWrapper: { flex: 1, alignItems: 'center', height: '100%', justifyContent: 'flex-end' }, barContainer: { width: '90%', height: '100%', alignItems: 'center', justifyContent: 'flex-end' }, bar: { width: '100%', borderRadius: 4, justifyContent: 'flex-start', alignItems: 'center', overflow: 'hidden' }, barValue: { paddingTop: 4, color: 'white', fontSize: 10, fontWeight: 'bold', textShadowColor: 'rgba(0, 0, 0, 0.5)', textShadowOffset: { width: 1, height: 1 }, textShadowRadius: 2 }, barLabel: { fontSize: 10, marginTop: 4, color: '#999' } });